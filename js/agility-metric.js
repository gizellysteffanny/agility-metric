...
gradient.addColorStop(1, '#FFB305');
gradient.addColorStop(0, '#FF5325');

var data = {
  labels: ['União', 'Organização', 'Geração de valor', 'Frequência', 'Funcionalidade', 'Flexibilidade', 'Comunicação', 'Sustentabilidade', 'Revisão', 'Simplicidade', 'Autoavaliação', 'Motivação'],
  datasets: [
    {
      label: 'Métricas de Agilidade',
      data: [4, 4, 3, 3, 3, 3, 4, 4, 3, 3, 3, 3],
...
var inputs = document.querySelectorAll('.agility-metric');
for(var i = 0; i < inputs.length; i++) {
  var input = inputs[i];
...
document.getElementById('download').addEventListener('click', function(e) {
  canvas.toBlob(function(blob) {
    saveAs(blob, 'agility-metric.png');
  });
});
